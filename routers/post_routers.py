from fastapi import APIRouter, Depends, HTTPException, Query
from app.schema.postDTO import Create_post, Post_update, PostResponse, PostListResponse, PostWithUserResponse, PostPerUser, PostSummaryResponse
from app.database.database import get_db
from app.database.models import Post, User
from sqlalchemy.orm import Session, joinedload, load_only
from app.dependencies.auth import get_current_user
from app.dependencies.permissions import admin_required
from typing import Optional, Literal
from sqlalchemy import asc, desc, or_ , func
from sqlalchemy.ext.asyncio import AsyncSession
import app.services.post_services as post_services

post_router = APIRouter(
    prefix = "/posts",
    tags = ["Posts"]
)

# CREATE A POST -->
@post_router.post("/create_post", status_code = 201, response_model = PostResponse )
async def create_post(payload : Create_post, db : AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):
    return await post_services.create_post(
        db,
        payload,
        current_user
    )

# GET ALL POSTS -->
@post_router.get("/all_posts", response_model = PostListResponse)
async def get_all_posts(
    title : Optional[str] = None,
    user_id : Optional[int] = None,
    sort_by : Literal[
        "id",
        "title",
        "user_id",
        "description"
    ] = "id",
    order : Literal[
        "asc",
        "desc"
    ] = "asc",
    limit : int = Query(10, ge = 1 , le = 100),
    offset : int = Query(0, ge = 0),
    db : Session = Depends(get_db), 
    search : Optional[str] = None,
    ):

    return await post_services.get_all_posts(
        db = db,
        title = title,
        user_id = user_id,
        search = search,
        sort_by = sort_by,
        order = order,
        limit = limit,
        offset = offset
    )

# GET POST WITH USER NAME AND EMAIL
@post_router.get("/posts/with-user", response_model = list[PostWithUserResponse])
async def get_post_with_user(db : Session = Depends(get_db)):
    return await post_services.get_post_with_user(db = db)

# TOTAL POSTS -->
@post_router.get("/posts/stats/total")
async def give_total_posts(db : AsyncSession = Depends(get_db)):
    return await post_services.total_posts(db=db)

# TOTAL POSTS BY SPECIFIC ID -->
@post_router.get("/posts/stats/total/{user_id}")
async def give_total_posts_by_user_id(user_id : int, db : AsyncSession = Depends(get_db)):
    return await post_services.total_posts_by_id(db=db, user_id=user_id)

# POSTS PER USER -->
@post_router.get("/posts/stats/user-posts", response_model = list[PostPerUser])
async def post_per_user(db : AsyncSession = Depends(get_db)):
    return await post_services.posts_per_user(db = db)

# USERS WHO HAVE ATLEAST 2 POSTS WITH PYTHON IN ITS TITLE -->
@post_router.get("/posts/stats/active-users")
async def active_users(db : AsyncSession = Depends(get_db)):
    return await post_services.active_users(db = db)

# GET POST BY ID -->
@post_router.get("/posts/{post_id}")
async def get_post_by_id(post_id : int, db : AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):
    return await post_services.get_post_by_id(
        db = db,
        post_id = post_id
    )

# DELETE A POST --> 
@post_router.delete("/deletepost/{post_id}")
async def delete_post(post_id : int, db : AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):
    return await post_services.delete_post(
        db = db,
        post_id= post_id,
        current_user=current_user
    )

# UPDATE A POST -->
@post_router.put("/update_post/{post_id}", response_model = PostResponse)
async def update_post(post_id : int,payload : Post_update, db : AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):

    return await post_services.update_post(
        db = db,
        post_id= post_id,
        payload=payload,
        current_user=current_user
    )

# All-users Stats -->
@post_router.get("/posts/stats/all-users")
async def all_users_stats(db : AsyncSession = Depends(get_db)):
    return await post_services.all_users_stats(db = db)

# USERS WITH 0 POSTS -->
@post_router.get("/posts/stats/all_with_zero_posts")
async def users_with_zero_posts(db : AsyncSession = Depends(get_db)):
    return await post_services.users_with_zero_posts(db=db)

# GET POST SUMMARY -->
@post_router.get("/summary", response_model = list[PostSummaryResponse])
async def post_summary(db : AsyncSession = Depends(get_db)):
    return await post_services.post_summary(db=db)